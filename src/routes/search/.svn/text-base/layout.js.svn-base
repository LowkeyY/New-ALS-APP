...
const RadioItem = Radio.RadioItem, flexItemLength = 3
...
module.exports = {
  layoutFilters: (filters = [], filterValues = {}, handleOnclick = () => {
    ...
    const result = []
    filters.map(filter => {
      let {key = '', cntype = '', items = [], title = ''} = filter, values = filterValues[key] || []
      if (key != '' && cntype != '') {
        switch (cntype) {
          case 'radio': {
            if (items.length > 0) {
              ...
              items.map((item, index) => {
                const {text = '', value = ''} = item, checked = values.includes(value)
